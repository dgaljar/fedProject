...
const EditUser = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setEmail(data.email || "");
          setRoles(Array.isArray(data.roles) ? data.roles[0] : "subscriber");
        } else {
          setMessage(
        ...
        setMessage(
          <div className="alert alert-danger">Failed to fetch user.</div>
        );
      }
    };
  ...
  if (id !== auth.id && auth.role !== "administrator") {
    return;
  }
