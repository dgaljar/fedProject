...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    const appStyles = document.querySelector('link[href="./App.css"]');
  ...
  const isActiveAdmin = (path) =>
    location.pathname === path ? "active-link-admin" : "";
              ...
              <ul className="d-flex justify-content-md-end align-items-center">
                <li>
                  <Link to="/admin/cs">
                    <FontAwesomeIcon
                      icon={faBell}
                      className="bell position-relative animate__animated animate__swing"
                    />
                  </Link>
                </li>
                <li>
        ...
        <div className={`sidebar ${toggle ? "show" : ""}`}>
          <ul className="sidebar-nav">
            <li>
              <Link
                to="/admin/cs"
                className={`${isActiveAdmin("/admin/cs")}`}
                onClick={closeSidebar}
              >
            ...
            <li>
              <Link
                to="/admin/categories"
                className={`${isActiveAdmin("/admin/categories")}`}
                onClick={closeSidebar}
              >
              ...
              <ul className={`sidebar-subnav ${isActive === 0 ? "show" : ""}`}>
                {auth.role === "subscriber" ? (
                  <li className="submenu-item">
                    <span>View Posts (restricted)</span>
                  </li>
                ) : (
                  <li className="submenu-item">
                    <Link
                      to="/admin/posts"
                      className={`${isActiveAdmin("/admin/posts")}`}
                      onClick={closeSidebar}
                    >
                      View All Posts
                    </Link>
                ...
                {auth.role === "subscriber" ? (
                  <li className="submenu-item">
                    <span>Add Post (Restricted)</span>
                  </li>
                ) : (
                  <li className="submenu-item">
                    <Link
                      to="/admin/posts/add"
                      className={`${isActiveAdmin("/admin/posts/add")}`}
                      onClick={closeSidebar}
                    >
                      Add Post
                    </Link>
            ...
            <li>
              <Link
                to="/admin/comments"
                className={`${isActiveAdmin("/admin/comments")}`}
                onClick={closeSidebar}
              >
            ...
            <li>
              ...
              <ul className={`sidebar-subnav ${isActive === 1 ? "show" : ""}`}>
                {auth.role === "subscriber" ? (
                  <li className="submenu-item">
                    <span>View All Users (Restricted)</span>{" "}
                  </li>
                ) : (
                  <li className="submenu-item">
                    <Link
                      to="/admin/users"
                      className={`${isActiveAdmin("/admin/users")}`}
                      onClick={closeSidebar}
                    >
                      View All Users
                    </Link>
                ...
                {auth.role === "administrator" ? (
                  <li className="submenu-item">
                    <Link
                      to="/admin/users/add"
                      className={`${isActiveAdmin("/admin/users/add")}`}
                      onClick={closeSidebar}
                    >
                      Add User
                    </Link>
            ...
            <li>
              <Link to="/admin/cs" onClick={closeSidebar}>
